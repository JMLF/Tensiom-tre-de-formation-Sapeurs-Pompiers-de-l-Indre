...
char DIA[3] = { '0','8','0' };
char PUL[3] = { '0','7','0' };
enum grandeurConstante {centaine , dizaine , unite  };
enum grandeurPin { millierP,centaineP, dizaineP, uniteP };
...
void my_timer_Ping(lv_timer_t* timer) {
    if (Bconnexion == false) {
        char ip[] ="192.168.4.3";//192.168.4.3";//10.187.52.43";
        Bconnexion = connexion(ip, 12345);
    }


}
void my_timer_envoyer(lv_timer_t* timer) {
    if (Bconnexion == true && Benvoyer ==false) {
         envoyer(DATA);// changer 
         Benvoyer = true;
         lv_msgbox_close(mbox1);
static const char* btns[] = { "Fermer","" };
 mbox1 = lv_msgbox_create(NULL, "Message envoye", "", btns, false);
...
static void event_off(lv_event_t * e)
{
...
lv_obj_t * btnOFF = lv_btn_create(winMenu);     /*Add a button the current screen*/
    lv_obj_align(btnOFF,LV_ALIGN_BOTTOM_MID,0,0);                            /*Set its position*/
    lv_obj_set_size(btnOFF, 320, 50);                          /*Set its size*/
    lv_obj_add_event_cb(btnOFF, event_off, LV_EVENT_ALL, NULL);           /*Assign a callback to the button*/

    lv_obj_t * labelOFF = lv_label_create(btnOFF);          /*Add a label to the button*/
    lv_label_set_text(labelOFF, LV_SYMBOL_POWER "   OFF   " LV_SYMBOL_POWER);                     /*Set the labels text*/
...
static void btn_event_send_ERREUR(lv_event_t* e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        if (Bconnexion == true) {
           char Erreur[2] = {'E','\0'};
	     Bconnexion = envoyer(Erreur);// changer
            //Bconnexion = false;
            mboxErreur = lv_msgbox_create(NULL, "Erreur vient d'etre envoyee", "", btns, false);
    ...
    if (code == LV_EVENT_CLICKED) {///////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if (Bconnexion == true) {// pour connexion
           ...
           // Bconnexion=false;
            ...
            Bconnexion = false;
            mbox1 = lv_msgbox_create(NULL, "Attente de connexion...", "", btns, false);
  	    lv_obj_add_event_cb(mbox1, event_cb, LV_EVENT_VALUE_CHANGED, NULL);
            lv_obj_center(mbox1);
...
static void btn_event_btnSecurite(lv_event_t* e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        char pinT[5];
        char tab[5];
        tab[millierP] = codeMillier;
        tab[centaineP] = codeCentaine;
        tab[dizaineP] = codeDizaine;
        tab[uniteP] = codeUnite;
        tab[4] = '\0';
        /*lire dans un txt*/
        FILE* f;
        f = fopen("/home/pi/lv_sim_vscode_sdl/SecuriteCode.txt","r");
        ...
        fclose(f);
        /*lire*/
        if ((tab[millierP] + tab[centaineP] + tab[dizaineP] + tab[uniteP]) == (pinT[millierP] + pinT[centaineP] + pinT[dizaineP] + pinT[uniteP])) { // 
            MenuPage();
            ...
            mboxPin = lv_msgbox_create(NULL, "Veuillez Reessayer svp", "", btns, false);
...
static void event_ChangerPin(lv_event_t* e) {
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        static const char* btns[] = { "Fermer","" };
        codePin[millierP] = codePinMillier^cle;
        codePin[centaineP] = codePinCentaine^cle;
        codePin[dizaineP] = codePinDizaine^cle;
        codePin[uniteP] = codePinUnite^cle;
        codePin[4] = '\0';

        FILE* f;
        f = fopen("/home/pi/lv_sim_vscode_sdl/SecuriteCode.txt", "w");
        ...
        if (f != NULL)  // si l'ouverture a réussi
        {
            ...
            mboxPin = lv_msgbox_create(NULL, "Le code pin vient d'etre change", "", btns, false);
            lv_obj_add_event_cb(mboxPin, event_mboxPin, LV_EVENT_VALUE_CHANGED, NULL);
            lv_obj_center(mboxPin);
        }
        else { // sinon
...
{
struct timeval tv;//09/06/2022
tv.tv_sec = 1;
tv.tv_usec = 0;
setsockopt(ids_client, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv, sizeof tv);
int taille = 0;
    if(data[0]=='E') taille = 1 ;
else taille = 11;
    if (false == send(ids_client, data, taille, 0)){
 return false;}
else   {
 return true;}
}
